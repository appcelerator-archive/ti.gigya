...
-	(void) GetUserInfo:(NSString *)pNotificationName  context:(id)pContext originalRequest:(GSObject *)pRequestParams	trace:(GSLogger	*)pTrace
{
	...
	[self SendRequest:@"socialize.getUserInfo" Params:nil UseHTTPS:NO context:pContext	trace:pTrace];
}
...
-	(void) SendRequest:(NSString	*)pMethod Params:(GSObject		*)pParams	UseHTTPS:(BOOL)bUseHTTPS context:(id)pContext	trace:(GSLogger	*)pTrace
{
	...
	GSRequest *pRequest = [[GSRequest alloc] initWithSessionToken:[self GetAccessToken] SecretKey:[self GetSecretKey] APIMethod:pMethod SendParams:pParams UseHTTPS:bUseHTTPS NotificationMessage:self.m_pNotificationMessage trace:pTrace serverTimestampSkewDelegate:self];
