...
- (void)openUrl:(NSString *)url
     ...
     httpMethod:(NSString *)httpMethod
       delegate:(id<GSFBRequestDelegate>)delegate {
  ...
  [_request release];
  _request = [[GSFBRequest getRequestWithParams:params
 ...
 */
- (BOOL)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
    if (tryFBAppAuth) {
      NSString *fbAppUrl = [GSFBRequest serializeURL:kFBAppAuthURL params:params];
      ...
      NSString *fbAppUrl = [GSFBRequest serializeURL:loginDialogURL params:params];
      didOpenOtherApp = [[UIApplication sharedApplication] openURL:[NSURL URLWithString:fbAppUrl]];
    }
		
  }
	return didOpenOtherApp;
}
...
safariAuth:(BOOL)bSafariAuth
{
  ...
  return [self authorizeWithFBAppAuth:YES safariAuth:bSafariAuth];
}
...
andDelegate:(id <GSFBDialogDelegate>)delegate {
  ...
  if (action == kLogin) {
    ...
    _fbDialog = [[GSFBLoginDialog alloc] initWithURL:dialogURL loginParams:params delegate:self];
  } else {
    if ([self isSessionValid]) {
      [params setValue:[self.accessToken stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]
                forKey:@"access_token"];
    }
    _fbDialog = [[GSFBDialog alloc] initWithURL:dialogURL params:params delegate:delegate];
  }
  ...
  [_fbDialog show];
	 ...
	 */
	- (void)request:(GSFBRequest*)request didFailWithError:(NSError*)error{
